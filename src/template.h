...
public:
	...
	float3 operator * ( const float3& operand ) const { return float3( x * operand.x, y * operand.y, z * operand.z ); }
	float3 operator / ( float operand ) const { return float3( x / operand, y / operand, z / operand ); }
...
{
public:
	union { struct { float x, y, z, w; }; struct { float r, g, b, a; }; float3 xyz; float3 rgb; float cell[4]; };
	float4() = default;
...
public:
	...
	static mat4 rotate( float3 v, float a );
...
public:
	...
	// data members
	union { __m128 bmin4 = _mm_set_ps( 1e34f, 1e34f, 1e34f, 0 ); float bmin[4]; float3 bmin3; };
	union { __m128 bmax4 = _mm_set_ps( -1e34f, -1e34f, -1e34f, 0 ); float bmax[4]; float3 bmax3; };
